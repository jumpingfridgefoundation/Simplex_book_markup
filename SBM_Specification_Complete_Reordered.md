...
The metadata framework provides essential publication information for cataloging, accessibility processing, and user experience customization. The metadata block is delimited by `<<` at the very top line and `>>` after the last metadata line. This block **must** appear at the beginning of every SBM document (MABF or the `book.mabf` file within an RABF archive).

```sbm
<<
meta:title=The SimplexBook Markup Specification>
meta:author=JumpingFridge Foundation>
meta:uid=978-0123456789>
meta:language=en-US>
meta:date=2025-11-22>
meta:version=2.0>
meta:publisher=JumpingFridge Foundation>
meta:description=A comprehensive technical specification for the SBM language.>
>>
```

The metadata block is now a mandatory, single block at the top of the file, enclosed by the `<<` and `>>` delimiters. Each metadata entry remains a standard SBM block, but the entire collection is parsed as a single unit before the main content.
...
- Custom fields must use the prefix `meta:custom:` followed by the specific field name
...
<<
meta:type=dada>
meta:title=Complete SBM Reference Guide>
meta:author=JumpingFridge Foundation>
meta:uid=978-0123456789>
meta:language=en-US>
meta:date=2025-11-22>
