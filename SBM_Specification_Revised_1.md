...
10. [Monolithic Accessible Text Format (MATF) Specification](#10-monolithic-accessible-text-format-matf-specification)
11. [Rich Accessible Book Format (RABF) Specification](#11-rich-accessible-book-format-RABFspecification)
11.5. [Hyper Audio Feature](#11-5-hyper-audio-feature)
12. [Comprehensive Syntax Showcase](#12-comprehensive-syntax-showcase)
13. [Tables and Figures Syntax](#13-tables-and-figures-syntax)
14. [Multi-Language Support](#14-multi-language-support)
15. [Parsing and Processing Rules](#15-parsing-and-processing-rules)
16. [Implementation Guidelines](#16-implementation-guidelines)
17. [Error and Success Codes](#17-error-and-success-codes)
18. [Advanced Formatting Features](#18-advanced-formatting-features)
19. [Implementation Examples](#19-implementation-examples)
20. [Validation and Quality Assurance](#20-validation-and-quality-assurance)
...
The development of MABF and RABF as separate formats came from the simple idea that different publications might need different approaches. **MABF** works reasonably well for text-focused works like novels and academic papers where the main content is textual. **RABF** adds multimedia capabilities including images and audio, while trying very hard to maintain accessibility standards for all users, with special attention to blind and visually impaired individuals. This separation hopes to keep simple publications lightweight while allowing more complex works to include enhanced features without sacrificing accessibility.
...
| :--- | :--- | :--- |
| **Tag Identifier** | A lowercase alphabetic string identifying the element type (e.g., `p`, `h1`, `img`). Compound tags use underscores (e.g., `figure_group`). | `p:` |
...
*   **Compound tags** are tags that group related elements or define complex structures. They are formed using underscore separation (e.g., `figure_group`). A compound tag is a tag that encapsulates a set of related child elements, forming a logical unit. For instance, a `figure_group` tag might contain an `img` tag and a `caption` tag.
...
- `application/x-sbm-text` - For .matf (Monolithic Accessible Text Format) files
...
audio:src="narration.mp3" disc:description="Welcome to the chapter">
```
...
The global style is declared using the `type` tag with the `book` parameter, which serves as the primary styling identifier for SBM documents:
...
Underscores connect multiple words in compound tag names (e.g., `figure_group`, `alt_text`). Screen readers should announce compound tags clearly, often with a slight pause between words to maintain comprehension of the complete tag name.
...
## 10. Monolithic Accessible Text Format (MATF) Specification

The **Monolithic Accessible Text Format (MATF)** is a simplified version of MABF designed for documents that do not require formal metadata, such as README files, simple notes, or short instructional texts.

### Key Characteristics

| Feature | MABF | MATF |
| :--- | :--- | :--- |
| **Metadata Block** | Mandatory (`<<` and ``) | Optional (Can be omitted) |
| **`type` Identifier** | Mandatory (`type:book>`) | Optional (Can be omitted) |
| **File Extension** | `.mabf` | `.matf` |
| **Use Case** | Formal publications, books | Simple text, readmes, notes |

### MATF Structure

An MATF document can begin immediately with content blocks, without any preceding metadata or structural tags.

```sbm
h1:Project Readme>
p:Welcome to the project. This document is in MATF format.>
ul:>
li:No metadata required.>
li:No type identifier required.>
li:Start writing immediately.>
>
```

This format is intended to lower the barrier to entry for creating accessible documents and is particularly useful for content that is frequently updated or does not require extensive bibliographic information.

---

## 11. Rich Accessible Book Format (RABF) Specification
...
The `book.mabf` file, whether stand-alone or within an RABF archive, is the single source of truth for all metadata. The metadata block is defined by the `<<` and `` delimiters at the beginning of the file. Note that the Monolithic Accessible Text Format (MATF) does not require a metadata block or a `type` identifier, allowing the author to begin writing content immediately.

### Multimedia Element Syntax

### 11.5. Hyper Audio Feature

The Hyper Audio feature is a key component of RABF, allowing authors to link specific audio files to inline text elements. This feature is designed to convey emotions, present character dialogue with distinct voices, or provide context-sensitive audio cues for blind and visually impaired users.

The syntax for Hyper Audio is an extension of the paragraph block:

```sbm
p: text_to_be_read=audio_file.mp3>
```

When a user's assistive technology reaches the text "text\_to\_be\_read", the linked audio file "audio\_file.mp3" will play. The text itself remains available for screen readers.

**Multi-Language Support with Variables:**

For multi-language support (MABF), the audio file name can be supplied via a variable defined in the language file:

```sbm
p: hello={valfile}>
```

If the value supplied for the file name does not contain a file extension (e.g., `.mp3`), the content will be treated as normal text and no audio will be played. This is a critical parsing rule to prevent errors.
...
| **Audio** | `audio` | `src`, `disc:description` | `audio:src="narration.mp3" disc:description="Welcome to Chapter 1.">` |
...
The `figure_group` tag enables the grouping of related elements, such as an image and its caption, to create a rich, accessible presentation.

```sbm
figure_group:id="chapter1-narration">
img:src="slide1.jpg" disc:desc="Title slide showing chapter overview.">
audio:src="audio1.mp3" disc:description="Introduction to the chapter.">
...
## 12. Comprehensive Syntax Showcase
...
figure_group:id="section-demonstration">
img:src="concept-visual.jpg" disc:desc="Visual representation of accessibility concepts.">
audio:src="concept-audio.mp3" disc:description="Explanation of accessibility design principles.">
>
```
...
## 15. Parsing and Processing Rules
...
## 18. Advanced Formatting Features
...
```sbm
figure_group:interactive="true" sync="time_based">
...
figure_group:id="demonstration-1">
